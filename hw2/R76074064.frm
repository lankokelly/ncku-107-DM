...
Begin VB.Form Partition 
   ...
   Begin VB.Label Label6 
      Caption         =   "-  按下特徵選取方法後，如要重新選擇離散化手法，請老師關掉視窗重新run"
      BeginProperty Font 
         Name            =   "微軟正黑體"
         Size            =   8.25
         Charset         =   0
...
Dim ent_probability(11) As Double
...
Private Sub EntropyBase_click()
    List1.Clear
    'discretize class(9th attribute)
    Dim ent_a1(1484, 3) 'j=1(sorted attr),j=2(sorted class)
    Dim ent_a2(1484, 3)
    Dim ent_a3(1484, 3)
    Dim ent_a4(1484, 3)
    Dim ent_a5(1484, 3)
    ...
    For X = 0 To 1483
        Select Case data4(X, 9)
            Case "CYT"
                ent_probability(1) = ent_probability(1) + 1
                data4(X, 9) = 1
            Case "NUC"
                ent_probability(2) = ent_probability(2) + 1
                data4(X, 9) = 2
            Case "MIT"
                ent_probability(3) = ent_probability(3) + 1
                data4(X, 9) = 3
            Case "ME3"
                ent_probability(4) = ent_probability(4) + 1
                data4(X, 9) = 4
            Case "ME2"
                ent_probability(5) = ent_probability(5) + 1
                data4(X, 9) = 5
            Case "ME1"
                ent_probability(6) = ent_probability(6) + 1
                data4(X, 9) = 6
            Case "EXC"
                ent_probability(7) = ent_probability(7) + 1
                data4(X, 9) = 7
            Case "VAC"
                ent_probability(8) = ent_probability(8) + 1
                data4(X, 9) = 8
            Case "POX"
                ent_probability(9) = ent_probability(9) + 1
                data4(X, 9) = 9
            Case Else
                ent_probability(10) = ent_probability(10) + 1
                data4(X, 9) = 10
        End Select
    Next
    
    For X = 1 To 10
        ent_probability(X) = ent_probability(X) / 1484
    Next
    
    'use bubble-sort to sort attribute and renew data4
    Dim Tag As Double
    For col = 1 To 8
        For X = 0 To 1483
            For Y = X To 1483
                If CDbl(data4(X, col)) > CDbl(data4(Y, col)) Then
                    'swift
                    Tag = data4(X, col)
                    data4(X, col) = data4(Y, col)
                    data4(Y, col) = Tag
                    'Change class
                    class = data4(X, 9)
                    data4(X, 9) = data4(Y, 9)
            ...
            Next Y
        If col = 1 Then
            ent_a1(X, 1) = data4(X, col)
            ent_a1(X, 2) = data4(X, 9)
        End If
        If col = 2 Then
            ent_a2(X, 1) = data4(X, col)
            ent_a2(X, 2) = data4(X, 9)
        End If
        If col = 3 Then
            ent_a3(X, 1) = data4(X, col)
            ent_a3(X, 2) = data4(X, 9)
        End If
        If col = 4 Then
            ent_a4(X, 1) = data4(X, col)
            ent_a4(X, 2) = data4(X, 9)
        End If
        If col = 5 Then
            ent_a5(X, 1) = data4(X, col)
            ent_a5(X, 2) = data4(X, 9)
        End If
        If col = 6 Then
            ent_a6(X, 1) = data4(X, col)
            ent_a6(X, 2) = data4(X, 9)
        End If
        If col = 7 Then
            ent_a7(X, 1) = data4(X, col)
            ent_a7(X, 2) = data4(X, 9)
        End If
        If col = 8 Then
            ent_a8(X, 1) = data4(X, col)
            ent_a8(X, 2) = data4(X, 9)
        End If
